...
# 2.  Data
# -----------------------------------------------------------------------------
cat("Loading Darmanis Human Brain dataset...\n")
sce <- DarmanisBrainData()
# Keep only the adult, single-type-confident populations: drop the fetal-stage
# labels (different developmental identity, not part of the adult cell-type
# panel) and "hybrid" (Darmanis et al.'s own catch-all for cells that could not
# be confidently assigned a single type -- not a real population). Same
# principle as the Segerstolpe "not applicable" filter.
keep        <- !is.na(colData(sce)$cell.type) &
  !colData(sce)$cell.type %in% c("fetal_quiescent", "fetal_replicating", "hybrid")
sce         <- sce[, keep]
counts_mat  <- as.matrix(counts(sce))                         # dense (all downstream ops densify anyway)
counts_mat  <- counts_mat[rowSums(counts_mat > 0) >= 10, ]
true_labels <- droplevels(factor(colData(sce)$cell.type))
...
write.csv(raw_df, "raw_results_DarmanisBrain.csv", row.names = FALSE)
...
write.csv(summary_df, "benchmark_summary_DarmanisBrain.csv", row.names = FALSE)
...
p_ari <- ggplot(summary_df, aes(K, ARI, colour = Method, group = Method)) +
  ...
  labs(title = "Feature selection - ARI (Darmanis Human Brain)",
...
p_nmi <- ggplot(summary_df, aes(K, NMI, colour = Method, group = Method)) +
  ...
  scale_x_continuous(breaks = TOP_K) + theme_bw(base_size = 13) +
  labs(title = "Feature selection - NMI (Darmanis Human Brain)",
       subtitle = "Error bars: bootstrap-seed SD (all methods bootstrapped identically)",
...
p_rt <- runtime_summary |>
  ...
  labs(title = "Wall-clock runtime - Darmanis Human Brain", x = NULL, y = "Time (seconds)")

ggsave("fig_ARI_DarmanisBrain.pdf",     p_ari, width = 8, height = 5)
ggsave("fig_NMI_DarmanisBrain.pdf",     p_nmi, width = 8, height = 5)
ggsave("fig_Runtime_DarmanisBrain.pdf", p_rt,  width = 6, height = 5)
...
sink("session_info_DarmanisBrain.txt"); print(si); sink()
