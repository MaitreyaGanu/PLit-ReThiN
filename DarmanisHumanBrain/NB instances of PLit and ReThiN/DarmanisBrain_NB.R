...
score_PLit_NB <- function(counts) {
  ...
  Sj <- tab[1L, ] - L0 - ((tab[2L, ] - 3) / 2) * log(n)         # d0 = 2 (mu, r) -> penalty (V_j-3)/2 * log n
...
write.csv(raw_df, "raw_results_DarmanisBrain_NB.csv", row.names = FALSE)
...
write.csv(summary_df, "benchmark_summary_DarmanisBrain_NB.csv", row.names = FALSE)
...
p_ari <- ggplot(summary_df, aes(K, ARI, colour = Method, group = Method)) +
  ...
  labs(title = "Feature selection - ARI (Darmanis Human Brain, NB instance)",
...
p_nmi <- ggplot(summary_df, aes(K, NMI, colour = Method, group = Method)) +
  ...
  scale_x_continuous(breaks = TOP_K) + theme_bw(base_size = 13) +
  labs(title = "Feature selection - NMI (Darmanis Human Brain, NB instance)",
       subtitle = "Error bars: bootstrap-seed SD (all methods bootstrapped identically)",
...
sink("session_info_DarmanisBrain_NB.txt"); print(si); sink()
