...
score_ReThiN <- function(counts, n_thin = 5L) {
  ...
  nz <- which(cv > 0); xnz <- cv[nz]                         # thin only nonzeros (binom(0,.)=0)
...
write.csv(raw_df, "raw_results_SegerstolpePancreas.csv", row.names = FALSE)
...
write.csv(summary_df, "benchmark_summary_SegerstolpePancreas.csv", row.names = FALSE)
...
write.csv(runtime_summary, "runtime_SegerstolpePancreas.csv", row.names = FALSE)
...
p_ari <- ggplot(summary_df, aes(K, ARI, colour = Method, group = Method)) +
  ...
  labs(title = "Feature selection - ARI (Segerstolpe 2016 Human Pancreas)",
...
p_nmi <- ggplot(summary_df, aes(K, NMI, colour = Method, group = Method)) +
  ...
  scale_x_continuous(breaks = TOP_K) + theme_bw(base_size = 13) +
  labs(title = "Feature selection - NMI (Segerstolpe 2016 Human Pancreas)",
       subtitle = "Error bars: bootstrap-seed SD (all methods bootstrapped identically)",
...
p_rt <- runtime_summary |>
  ...
  labs(title = "Wall-clock runtime - Segerstolpe 2016 Human Pancreas", x = NULL, y = "Time (seconds)")

ggsave("fig_ARI_SegerstolpePancreas.pdf",     p_ari, width = 8, height = 5)
ggsave("fig_NMI_SegerstolpePancreas.pdf",     p_nmi, width = 8, height = 5)
ggsave("fig_Runtime_SegerstolpePancreas.pdf", p_rt,  width = 6, height = 5)
...
sink("session_info_SegerstolpePancreas.txt"); print(si); sink()
