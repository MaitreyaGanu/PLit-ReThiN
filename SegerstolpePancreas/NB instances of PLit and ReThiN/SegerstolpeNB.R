# =============================================================================
# Feature-selection benchmark  --  Segerstolpe 2016 Human Pancreas
...
# 2.  Data
# -----------------------------------------------------------------------------
cat("Loading Segerstolpe 2016 Human Pancreas dataset...\n")
sce         <- SegerstolpePancreasData()
# Remove cells without a meaningful cell-type label (doublets / low-quality)
keep        <- !is.na(colData(sce)[["cell type"]]) & colData(sce)[["cell type"]] != "not applicable"
sce         <- sce[, keep]
counts_mat  <- as.matrix(counts(sce))                         # dense (all downstream ops densify anyway)
counts_mat  <- counts_mat[rowSums(counts_mat > 0) >= 10, ]
true_labels <- factor(colData(sce)[["cell type"]])
...
write.csv(raw_df, "raw_results_SegerstolpePancreas_NB.csv", row.names = FALSE)
...
write.csv(summary_df, "benchmark_summary_SegerstolpePancreas_NB.csv", row.names = FALSE)
...
write.csv(runtime_summary, "runtime_SegerstolpePancreas_NB.csv", row.names = FALSE)

# NOTE: cross-method statistical significance (Friedman + post-hoc) is computed
#       ACROSS the 7 datasets, using one (method, dataset) score each -- not on
#       k-means seeds within a single dataset (that would be pseudoreplication).
...
p_ari <- ggplot(summary_df, aes(K, ARI, colour = Method, group = Method)) +
  ...
  labs(title = "Feature selection - ARI (Segerstolpe 2016 Human Pancreas, NB instance)",
...
p_nmi <- ggplot(summary_df, aes(K, NMI, colour = Method, group = Method)) +
  ...
  scale_x_continuous(breaks = TOP_K) + theme_bw(base_size = 13) +
  labs(title = "Feature selection - NMI (Segerstolpe 2016 Human Pancreas, NB instance)",
       subtitle = "Error bars: bootstrap-seed SD (all methods bootstrapped identically)",
...
p_rt <- runtime_summary |>
  ...
  labs(title = "Wall-clock runtime - Segerstolpe 2016 Human Pancreas (NB instance)", x = NULL, y = "Time (seconds)")

ggsave("fig_ARI_SegerstolpePancreas_NB.pdf",     p_ari, width = 8, height = 5)
ggsave("fig_NMI_SegerstolpePancreas_NB.pdf",     p_nmi, width = 8, height = 5)
ggsave("fig_Runtime_SegerstolpePancreas_NB.pdf", p_rt,  width = 6, height = 5)
...
sink("session_info_SegerstolpePancreas_NB.txt"); print(si); sink()