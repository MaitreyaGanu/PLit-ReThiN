...
SCORERS <- list(
  ReThiN_NB = score_ReThiN_NB, PLit_NB = score_PLit_NB, scran_HVG = score_scran,
...
write.csv(raw_df, "raw_results_TianCellBench_NB.csv", row.names = FALSE)
...
write.csv(summary_df, "benchmark_summary_TianCellBench_NB.csv", row.names = FALSE)
...
p_ari <- ggplot(summary_df, aes(K, ARI, colour = Method, group = Method)) +
  ...
  labs(title = "Feature selection - ARI (Tian CellBench 10x, NB instance)",
...
p_nmi <- ggplot(summary_df, aes(K, NMI, colour = Method, group = Method)) +
  ...
  scale_x_continuous(breaks = TOP_K) + theme_bw(base_size = 13) +
  labs(title = "Feature selection - NMI (Tian CellBench 10x, NB instance)",
       subtitle = "Error bars: bootstrap-seed SD (all methods bootstrapped identically)",
...
p_rt <- runtime_summary |>
  ...
  labs(title = "Wall-clock runtime - Tian CellBench 10x (NB instance)", x = NULL, y = "Time (seconds)")
...
sink("session_info_TianCellBench_NB.txt"); print(si); sink()