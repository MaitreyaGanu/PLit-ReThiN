...
# -----------------------------------------------------------------------------
# 2.  Data  (CellBench 10x; downloads once from the official GitHub repo)
# -----------------------------------------------------------------------------
data_file <- "sincell_with_class.RData"
if (!file.exists(data_file)) {
  cat("Downloading CellBench data (one-time, ~30 MB)...\n")
  download.file(
    "https://github.com/LuyiTian/CellBench_data/raw/master/data/sincell_with_class.RData",
    destfile = data_file, mode = "wb")
}
cat("Loading CellBench 10x data...\n")
load(data_file)                       # -> sce_sc_10x_qc, sce_sc_CELseq2_qc, sce_sc_Dropseq_qc
sce        <- sce_sc_10x_qc
...
true_labels <- if ("cell_line" %in% colnames(colData(sce))) {
  factor(colData(sce)$cell_line)
} else if ("group" %in% colnames(colData(sce))) {
  factor(colData(sce)$group)
} else {
  stop("Cannot find label column; set true_labels manually.")
}
...
KMEANS_SEEDS  <- 1:30       # standardised to 1:30 across ALL 7 datasets (Issue #3)
KMEANS_NSTART <- 25L

# -----------------------------------------------------------------------------
# 4.  Core scorers   (counts -> named per-gene score; higher = more important)
#     None sees the labels.  No bootstrap inside -- the wrapper adds it.
# -----------------------------------------------------------------------------
...
score_ReThiN <- function(counts, n_thin = 5L) {               # proposed (= SPARC core: rho_bar)
...
score_PLit <- function(counts) {                              # proposed (= PCG core: Sj)
  ...
  Sj <- tab[1L, ] - L0 - ((tab[2L, ] - 2L) / 2) * log(n)            # = L0_code - L1_code (verified vs spec)
  Sj[lam == 0] <- -Inf
...
write.csv(raw_df, "raw_results_TianCellBench.csv", row.names = FALSE)
...
write.csv(summary_df, "benchmark_summary_TianCellBench.csv", row.names = FALSE)
...
write.csv(runtime_summary, "runtime_TianCellBench.csv", row.names = FALSE)
...
p_ari <- ggplot(summary_df, aes(K, ARI, colour = Method, group = Method)) +
  ...
  labs(title = "Feature selection - ARI (Tian CellBench 10x)",
...
p_nmi <- ggplot(summary_df, aes(K, NMI, colour = Method, group = Method)) +
  ...
  scale_x_continuous(breaks = TOP_K) + theme_bw(base_size = 13) +
  labs(title = "Feature selection - NMI (Tian CellBench 10x)",
       subtitle = "Error bars: bootstrap-seed SD (all methods bootstrapped identically)",
...
p_rt <- runtime_summary |>
  ...
  labs(title = "Wall-clock runtime - Tian CellBench 10x", x = NULL, y = "Time (seconds)")
ggsave("fig_ARI_TianCellBench.pdf",     p_ari, width = 8, height = 5)
ggsave("fig_NMI_TianCellBench.pdf",     p_nmi, width = 8, height = 5)
ggsave("fig_Runtime_TianCellBench.pdf", p_rt,  width = 6, height = 5)
...
sink("session_info_TianCellBench.txt"); print(si); sink()