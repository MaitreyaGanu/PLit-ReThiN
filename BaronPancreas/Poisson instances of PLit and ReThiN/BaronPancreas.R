# =============================================================================
# Feature-selection benchmark  --  Baron Human Pancreas
...
#           symmetric error bars.  Cross-method significance is computed across the 7 datasets.
...
#     FIX: ntop = length(top_genes) so runPCA does NOT silently re-select the
...
write.csv(raw_df, "raw_results_BaronPancreas.csv", row.names = FALSE)
...
write.csv(summary_df, "benchmark_summary_BaronPancreas.csv", row.names = FALSE)
...
write.csv(runtime_summary, "runtime_BaronPancreas.csv", row.names = FALSE)
...
p_ari <- ggplot(summary_df, aes(K, ARI, colour = Method, group = Method)) +
  ...
  labs(title = "Feature selection - ARI (Baron Human Pancreas)",
...
p_nmi <- ggplot(summary_df, aes(K, NMI, colour = Method, group = Method)) +
  ...
  scale_x_continuous(breaks = TOP_K) + theme_bw(base_size = 13) +
  labs(title = "Feature selection - NMI (Baron Human Pancreas)",
       subtitle = "Error bars: bootstrap-seed SD (all methods bootstrapped identically)",
...
p_rt <- runtime_summary |>
  ...
  labs(title = "Wall-clock runtime - Baron Human Pancreas", x = NULL, y = "Time (seconds)")

ggsave("fig_ARI_BaronPancreas.pdf",     p_ari, width = 8, height = 5)
ggsave("fig_NMI_BaronPancreas.pdf",     p_nmi, width = 8, height = 5)
ggsave("fig_Runtime_BaronPancreas.pdf", p_rt,  width = 6, height = 5)
...
sink("session_info_BaronPancreas.txt"); print(si); sink()