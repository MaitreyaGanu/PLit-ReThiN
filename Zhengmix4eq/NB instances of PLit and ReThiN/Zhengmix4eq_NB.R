...
# -----------------------------------------------------------------------------
# 2.  Data  (4 purified populations mixed in equal proportions)
# -----------------------------------------------------------------------------
...
write.csv(raw_df, "raw_results_ZhengMix4eq_NB.csv", row.names = FALSE)
...
write.csv(summary_df, "benchmark_summary_ZhengMix4eq_NB.csv", row.names = FALSE)
...
write.csv(runtime_summary, "runtime_ZhengMix4eq_NB.csv", row.names = FALSE)
...
p_ari <- ggplot(summary_df, aes(K, ARI, colour = Method, group = Method)) +
  ...
  labs(title = "Feature selection - ARI (Zheng 2017, Zhengmix4eq, NB instance)",
...
p_nmi <- ggplot(summary_df, aes(K, NMI, colour = Method, group = Method)) +
  ...
  scale_x_continuous(breaks = TOP_K) + theme_bw(base_size = 13) +
  labs(title = "Feature selection - NMI (Zheng 2017, Zhengmix4eq, NB instance)",
       subtitle = "Error bars: bootstrap-seed SD (all methods bootstrapped identically)",
...
p_rt <- runtime_summary |>
  ...
  labs(title = "Wall-clock runtime - Zheng 2017 FACS PBMC (Zhengmix4eq, NB instance)", x = NULL, y = "Time (seconds)")
ggsave("fig_ARI_ZhengMix4eq_NB.pdf",     p_ari, width = 8, height = 5)
ggsave("fig_NMI_ZhengMix4eq_NB.pdf",     p_nmi, width = 8, height = 5)
ggsave("fig_Runtime_ZhengMix4eq_NB.pdf", p_rt,  width = 6, height = 5)
...
sink("session_info_ZhengMix4eq_NB.txt"); print(si); sink()
